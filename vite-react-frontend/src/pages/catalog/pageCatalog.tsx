...
export type PreviewKind = 'chart' | 'table' | 'timeline' | 'calendar' | 'cards';
...
export function getPageMeta(to: string, label: string, groupTitle: string): PageMeta {
  return (
    PAGE_META[to] ?? {
      description: `${label} — a ${groupTitle.toLowerCase()} workspace (skeleton for now).`,
      preview: { kind: 'table', label: groupTitle },
    }
  );
}
